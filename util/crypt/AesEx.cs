...
namespace util.crypt
{
    public static class AesEx
    {
        ...
        // cbc TransformBlock test
        //var aes = new byte[32].aesInit(CipherMode.CBC, PaddingMode.PKCS7, new byte[16]);
        //var enc = aes.CreateEncryptor();
        //var src = new byte[32];
        //var dst = new byte[40];
        //var len = enc.TransformBlock(src, 0, src.Length, dst, 0);
        //len.debug();
        //    byte[] last = enc.TransformFinalBlock(src, 0, 0);
        //last.hex().debug();
        //last.Length.debug();
    }
}
